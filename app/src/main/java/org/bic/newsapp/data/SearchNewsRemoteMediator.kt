...
class SearchNewsRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            ...
            delay(3000)
            val serviceSearchResult = response.articles

            val bookmarkArticles = newsArticleDao.getAllBreakingNewsArticles().first()

            val searchResultArticles = serviceSearchResult.map{
                serverSearchResultArticle -> val isBookmarked = bookmarkArticles.any{
                    bookmarkArticle -> bookmarkArticle.url == serverSearchResultArticle.url
            ...
            newsArticleDb.withTransaction {
                //Transactions are used for when either all operations succeed or none of them are applied
                ...
                val searchResults = searchResultArticles.map {
                    article ->
            ...
            return MediatorResult.Success(endOfPaginationReached = serviceSearchResult.isEmpty())
